...
conn = pypyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER=;DATABASE=;UID=;PWD=')
...
def list():
    ...
    query1 = "SELECT count(*) FROM QUAKES WHERE LOCATIONSOURCE = '" + str(locationsrc) + "' AND MAG between '" + str(range1) + "' AND '" + str(range2) + "'"
...
def pie():
    ...
    query8 = "select count(*) from QUAKES where MAG > 5.0 and depthError > 5"
    cursor.execute(query8)
    r8 = cursor.fetchall()
    query9 = "select count(*) from QUAKES where MAG > 5.0 and depthError < 5"
